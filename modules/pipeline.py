...
class ReasoningRecommender(nn.Module):
    ...
    def save_state_dict(
        ...
        torch.save(lora_state_dict, os.path.join(lora_path, 'lora.pt'))
        torch.save(self.rec_head.state_dict(), os.path.join(rec_path, 'rec.pt'))
        adapter_state_dict = {
            'lm_head': self.lm_head.state_dict(),
            'talk_head': self.talk_head.state_dict()
        }
        torch.save(adapter_state_dict, os.path.join(adapter_path, 'adapter.pt'))
    ...
    def load_state_dict(
        ...
        lora_state_dict = torch.load(os.path.join(lora_path, 'lora.pt'))
        set_peft_model_state_dict(self.llm, lora_state_dict)
        self.rec_head.load_state_dict(torch.load(os.path.join(rec_path, 'rec.pt')))
        adapter_state_dict = torch.load(os.path.join(adapter_path, 'adapter.pt'))
        self.lm_head.load_state_dict(adapter_state_dict['lm_head'])
        self.talk_head.load_state_dict(adapter_state_dict['talk_head'])
    ...
    def forward(
        ...
        if self.enable_reasoning:
            ...
            logits = self.lm_head(mixed_hidden_states)
        return logits, rec_logits, outputs_after
