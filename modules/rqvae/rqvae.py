...
from modules.rqvae.encoder import MLP
from modules.rqvae.loss import ReconstructionLoss, QuantizeLoss
from modules.rqvae.normalize import l2norm
from modules.rqvae.quantize import Quantize, QuantizeForwardMode, QuantizeDistance
...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        self.layers = nn.ModuleList(modules=[
            Quantize(
                ...
                distance_mode=codebook_distance_mode,
        ...
        self.encoder = MLP(
            ...
            normalize_type=codebook_normalize_type
        )
    ...
    def get_embeddings_from_ids(
        ...
        for i, layer in enumerate(self.layers):
            emb = layer.get_embeddings_from_ids(sem_ids[:, i, :])
            embs.append(emb)
        embs = rearrange(embs, "b h d -> h d b")
    ...
    def forward(self, batch, gumbel_t: float) -> RqVaeComputedLosses:
        x = batch.x
