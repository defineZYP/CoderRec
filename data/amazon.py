...
class AmazonReviews(InMemoryDataset, PreprocessingMixin):
    ...
    def download(self):
        if os.path.exists(os.path.join(self.root, f'raw/{self.split}')):
            return 
        path = os.path.join(os.path.join(self.root, 'raw'), self.gdrive_filename)
    ...
    def process(self, max_seq_len=20, model=None, tokenizer=None):
        data = HeteroData()
        ...
        with torch.no_grad():
            with tqdm.tqdm(sentences) as bar:
                for sentence in bar:
                    item_emb = self._encode_text_feature(sentence, model=model, tokenizer=tokenizer).detach().cpu()
                    item_embs.append(item_emb)

        item_emb = torch.stack(item_embs)
