...
class PreprocessingMixin:
    ...
    @staticmethod
    def _encode_text_feature(text_feat, model=None, tokenizer=None):
        # 经典t5模型
        # if model is None:
        ...
        # outputs = model(**model_inputs, output_hidden_states=True).hidden_states[-1][:, -1, :]
        outputs = model(**model_inputs, output_hidden_states=True).hidden_states[-1][:, -1, :].squeeze()
        return outputs
